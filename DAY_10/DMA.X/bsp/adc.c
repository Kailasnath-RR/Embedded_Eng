...
bool ADC_SetConfiguration(ADC_CONFIGURATION configuration)
{
    ...
    {
        AD1CON1bits.ADON = 0; //make sure this reg is 0 before changing configs
            AD1CON1bits.FORM = 0; //stores the data in integer format()more like interprets it as)
            AD1CON1bits.AD12B = 0; //10 bits resolution (stores sample data as 10 bits in reg)
            AD1CON1bits.SSRC = 2; //timer 3 starts the conversion and ends it too
            AD1CON1bits.ASAM = 1; //auto sample

            AD1CON2bits.CHPS = 0; //only 1 channel req for adc which is channel 0
            AD1CON2bits.SMPI = 0; // interrupt after every conversion

            AD1CON2bits.BUFM = 0;

            AD1CON3bits.ADRC = 0; //which timing source to use for adc (system clock here)
            ...
            AD1CON1bits.ADDMABM = 0; //buffer mode

            AD1CHS0bits.CH0SA = 12; //sets which channel to use for adc     
